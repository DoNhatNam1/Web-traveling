...
const SearchHomePage = () => {
    const { searchData, isSearching, isSearchingLoading } = useContext(Context);
    ...
    useEffect(() =>{
      ...
      useEffect(() => {
        if (isSearching && searchData) {
          const filteredResults = Data.filter((trip) => {
            return (
              String(trip.DiemKhoiHanh).toLowerCase().indexOf(String(searchData.fromLocation).trim().toLowerCase()) !== -1 &&
              String(trip.DiemDen).toLowerCase().indexOf(String(searchData.toLocation).trim().toLowerCase()) !== -1 &&
              trip.NgayKhoiHanh === moment(searchData.departureDate, 'YYYY-MM-DD').format('DD-MM-YYYY') &&
              trip.DonGia <= searchData.maxPrice + 500000 &&
              trip.DonGia >= searchData.maxPrice - 500000
            );
          });
