...
import BackEndChangePassDashboard from '../../../../apis/user-dashboard-changepassword-backend-url'
...
const UserChangePasswordBody = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await BackEndChangePassDashboard.get(`/${id}`);
      console.log(response.data.datauseraccout);
      SetPasswordFromDb(response.data.datauseraccout.useraccoutpassonly);
      setHasFetched(true);
    } catch (err) {
      console.log(err);
    }
    ...
    e.preventDefault();
      ...
      console.log(matchUserAccountPassOnly); 
       ...
       if (matchUserAccountPassOnly && newPassword === passwordConfirm && newPassword !== '' && passwordConfirm !== ''){
        ...
        const updatedUserAccount = await BackEndChangePassDashboard.put(`/${id}`, {
          Pass: hashedPassword
        }).then((response) => {
          console.log(response.data.datauseraccout);
          setDashboardShowChangePassSuccess(true)
          navigateTo(`/userdashboard/${id}/maindashboard`);
        }).catch((err) => {
          console.log(err);
        });
      ...
      setLoginStatus('On');
        ...
        navigateTo(`/userdashboard/changepassword/${id}`);
    } else if (
      (matchUserAccountPassOnly && newPassword !== passwordConfirm && newPassword !== '' && passwordConfirm !== '') || 
      (matchUserAccountPassOnly && newPassword !== passwordConfirm && newPassword === '' && passwordConfirm === '') || 
      (matchUserAccountPassOnly && newPassword !== passwordConfirm && newPassword !== '' && passwordConfirm === '') || 
